...
function memberToRow(m) {
  return {
    ...
    phone: m.phone ?? null,
    password_hash: m.password ?? m.passwordHash ?? null,
    google_sub: m.googleId ?? m.googleSub ?? null,
    ...
    is_guest: m.isGuest === true,
    sort_order: typeof m.sortOrder === 'number' ? m.sortOrder : 0,
  };
}
...
function eventToRow(e) {
  return {
    ...
    sort_order: typeof e.sortOrder === 'number' ? e.sortOrder : 0,
...
function blogToRow(b) {
  return {
    ...
    youtube_id: b.youtubeId ?? null,
    youtube_url: b.youtubeUrl ?? null,
    sort_order: typeof b.order === 'number' ? b.order : 0,
  };
}
...
// ---------- password_resets mappers ----------
function pwResetFromRow(r) {
  return {
    token: r.token,
...
async function readAll() {
  ...
  const [mem, ev, bl, bo, ms, st, op, pw] = await Promise.all([
  ...
  const first = [mem, ev, bl, bo, ms, op, pw].find((r) => r.error);
  ...
  return {
    ...
    passwordResets: (pw.data || []).map(pwResetFromRow),
    interviews: [],
    groupChats: [],
  };
}
...
async function writeAll(data) {
  ...
  const passwordResets = (data.passwordResets || []).map(pwResetToRow);
  ...
  const results = await Promise.all([
    ...
    passwordResets.length ? supabase.from('password_resets').upsert(passwordResets) : { error: null },
  ]);
  ...
  // password_resets は token PK で同期削除
  await syncDeletesByKey('password_resets', 'token', passwordResets.map((p) => p.token));
}

async function syncDeletesByKey(table, keyColumn, keys) {
  const { data: existing, error } = await supabase.from(table).select(keyColumn);
  if (error) throw error;
  const keep = new Set(keys);
  const toDelete = (existing || []).map((r) => r[keyColumn]).filter((k) => !keep.has(k));
  if (toDelete.length === 0) return;
  const { error: delErr } = await supabase.from(table).delete().in(keyColumn, toDelete);
